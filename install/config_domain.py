...
print 'Creando keyfiles del AdminServer';
storeUserConfig(admin_server_config_file,admin_server_key_file);
managed_servers = p_servers.split(',');

for i, managed_server in enumerate(managed_servers):
    cd('/');
    print 'Creando configuracion de reports server y tools ' + managed_server;
    createReportsServerInstance(instanceName='repsrv' + str(i+1),machine='machine_' + managed_server);
    createReportsToolsInstance(instanceName='reptools' + str(i+1),machine='machine_' + managed_server);

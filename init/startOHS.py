...
for sm in servers:
    if (sm.getComponentType() == 'OHS'):
        if (socket.gethostbyname(sm.getMachine().getNodeManager().listenAddress) == socket.gethostbyname(node_manager_listen_address)):
            serverstatus=nmServerStatus(serverName=sm.getName(), serverType=sm.getComponentType());
            if (serverstatus == 'SHUTDOWN' or serverstatus == 'FAILED'):
                print 'START SERVER ' + sm.getName();
                nmStart(serverName=sm.getName(),serverType='OHS');
exit();
